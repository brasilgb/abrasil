...
@section('content')
    ...
    <div class="card bg-light shadow-sm">
        ...
        <div class="card-header clearfix">
            <a href="{{ route('agendas.index') }}" class="btn btn-primary float-left"><i class="fa fa-angle-left"></i>
                Voltar</a>
            ...
            <form action="{{ route('agendas.update', [$agenda->id_agenda]) }}" method="POST" autocomplete="off">
                ...
                <div class="form-group row">
                    <label class="col-sm-2 col-form-label" for=""><i class="fa fa-asterisk text-danger small"></i>
                        Cliente</label>
                    <div class="col-sm-10">
                        <input id="" type="text" class="cliente form-control" name="cliente"
                            value="{{ old('cliente', $agenda->clientes->cliente) }}">
                        <input id="" type="hidden" class="cliente_id form-control" name="cliente_id"
                            value="{{ old('cliente_id', $agenda->cliente_id) }}">
                        <input id="" type="hidden" class="cliente_id form-control" name="id_agenda"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input id="dateform" type="text" class="form-control" name="data"
                            value="{{ old('data', date('d/m/Y', strtotime($agenda->data))) }}">
                        @error('data')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" name="servico"
                            value="{{ old('servico', $agenda->servico) }}">
                        @error('servico')
                            <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> {{ $message }}
                            </div>
                        @enderror
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea type="text" class="form-control"
                            name="detalhes">{{ old('detalhes', $agenda->detalhes) }}</textarea>
                        @error('detalhes')
                            <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> {{ $message }}
                            </div>
                        @enderror
                        ...
                        <select class="custom-select my-1 mr-sm-2" name="tecnico_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('tecnico_id', $user->id) == $agenda->tecnico_id ? 'selected' : '' }}>
                                    {{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('tecnico_id')
                            <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> {{ __('O técnico deve ser selecionado!') }}
                            </div>
                        @enderror
                    </div>
                </div>
                    ...
                    <div class="col-sm-10">
                        <select class="custom-select my-1 mr-sm-2" name="status">
                            @foreach ($status as $key => $value)
                                <option value="{{ $key }}"
                                    {{ old('status', $key) == $agenda->status ? 'selected' : '' }}>{{ $value }}
                                </option>
                            @endforeach
                        </select>
                        @error('status')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea type="text" class="form-control"
                            name="observacoes">{{ old('observacoes', $agenda->observacoes) }}</textarea>
                    </div>
                </div>

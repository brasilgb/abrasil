...
<div class="card bg-light">
    <div class="card-header clearfix">
        <a href="{{ route('pecas.index') }}" class="btn btn-primary float-left"><i class="fa fa-angle-left"></i> Voltar</a>
    ...
    <div class="card-body">
        ...
        <form action="{{ route('pecas.store') }}" method="POST" autocomplete="off">
            ...
            <div class="form-group row">
                <label class="col-sm-2 col-form-label" for=""><i class="fa fa-asterisk text-danger small"></i> Peça:</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="peca" value="{{old('peca')}}">
                    @error('peca')
                    <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> O campo peça deve ser preenchido!</div>
                    @enderror
                </div>
            </div>
            <div class="form-group row">
            ...
            @endphp
            <div class="form-group row">
                <label class="col-sm-2 col-form-label" for=""><i class="fa fa-asterisk text-danger small"></i> Situação:</label>
                <div class="col-sm-10">
                    <select class="custom-select my-1 mr-sm-2" name="situacao">
                        @foreach ($situacao as $key => $value)
                        <option value="{{ $key }}" {{ old('situacao') == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('situacao')
                    <div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> {{ $message }}</div>
